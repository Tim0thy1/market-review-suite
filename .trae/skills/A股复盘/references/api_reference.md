...
> **盘中模式**：westock 类命令（quote / market-overview / changedist / sector ranking / hot board）本就返回实时快照，直接调用即可；neodata 查询去掉具体日期，改用「今日 / 当前 / 盘中」（如「今日A股盘中领涨板块和热门个股有哪些，为什么上涨」）。详见 SKILL.md「盘中概览模式」章节。
